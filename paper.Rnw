...
\par One set of models for dynamic social networks are the stochastic actor-oriented models (SAOMs) introduced by \citet{saompaper}. These models are fundamentally different from other social network models because they incorporate network {\emph and} actor statistics into the model, when other models only rely on  the network statistics to determine change. Allowing the actor-level statistics to change the structure of the network leads to a more intuitive model for social network change. In the ``real" world we expect people with common interests to be more likely to form relationships. SAOMs allow us to incorporates this intuition  in the modeling process. 
...
There are two primary methods used to visualize networks: node-link diagrams and matrix visualization.
%\hh{As a toy example, let us assume that we have five nodes, $\{1,2,3,4,5\}$, connected by some edges: XXX list edges as $1 \mapto 2, 2 \mapto 4, ...$ }
We will use the toy data set shown below to demonstrate these two methods.

\hh{XXX describe the dataset in words, but not with the code. }
<<toyex>>=
set.seed(81393)
...
outdegree  & $s_{i1}(x) = \sum_j x_{ij}$ & \includegraphics[width=3cm]{img/outdegree}\\
reciprocity  & $s_{i2}(x) = \sum_j x_{ij}x_{ji}$ & \includegraphics[width=3cm]{img/reciprocity}\\
transitive triplets  & $s_{i3}(x) = \sum_{j,h}& \includegraphics[width=3cm]{img/transtrip} x_{ij}x_{jh}x_{ih}$ \\
\textbf{Covariate Effects}\\
covariate-alter  & $s_{i4}(x) = \sum_j x_{ij}z_j$ & \includegraphics[width=3cm]{img/covaralter} \\
covariate-ego  & $s_{i5}(x) = z_i\sum_j x_{ij}$ & \includegraphics[width=3cm]{img/covearego} \\
