...
Because we require binary edges for our models, we focus only on very strong collaborations. The networks we constructed for the four senates during President Obama's administration are shown in Figure~\ref{fig:senateplot} so for our collaboration networks, $x$, edges are defined as 
\[
  x_{ij} =
  \begin{cases}
                                   1 &  \text{if } WPC_{ij} > 0.25 \\
                                   0 & \text{if } WPC_{in \leq 0.25}.
  \end{cases}
\]
