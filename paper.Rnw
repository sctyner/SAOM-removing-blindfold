...
We also explore simulations from these different models given parameter values. Using the means from the 1,000 fitted parameter values, we simulated 1,000 observations from each of our three models. We condition on the first friendship network observation, and the second and third observations are simulated from the SAOM models estimated previously. From these simulations, we first create a visualization that represents an ``average" network, we count occurences of each possible edge in the simulations, resulting in a summary network of simulations. 
...
distmat <- distmat[,-1]
distmat <- as.matrix(sqrt(1/distmat))
distmat[is.na(distmat)] <- 0
ggplot(data = M1avgcov) + # edge appears > 50% of the time. (is this actually what that is????) 
