...
to_label <- c("Joseph R. Biden Jr.", "John S. McCain", "Ted Cruz","Marco Rubio",
  ...
  "Harry M. Reid", "Hillary Rodham Clinton")
seobama$label <- as.factor(ifelse(seobama$source %in% to_label, seobama$source, ""))
levels(seobama$label) <- c("", "Bernie Sanders", "Harry Reid", "Hillary Clinton", 
                           "Jim Webb", "John McCain", "Joe Biden", "Lindsey Graham",
                           "Marco Rubio", "Mitch McConnel", "Rand Paul", "Ted Cruz")
set.seed(56049382)
ggplot(data = seobama) + 
