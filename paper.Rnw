...
Thus, the rate of change between any two states, $x^b$ and $x^c$, that differ by only one tie $x_{ij}$, is the product of the rate at which actor $i$ gets to change a tie and the probability that the tie that will change is the tie to node $j$. Estimating the parameters in these models is difficult, but thanks to the SIENA software, we have an accessible way to fit SAOMs to network observations. 
...
senM1est %>% gather(parameter, estimate, Rate:recip) %>% select(Sim, Model, parameter, estimate) -> senM1est2 
senM2est %>% gather(parameter, estimate, Rate:jumpXTransTrip) %>% select(Sim, Model, parameter, estimate) -> senM2est2 
senM3est %>% gather(parameter, estimate, Rate:nbrDist2twice) %>% select(Sim, Model, parameter, estimate) -> senM3est2 
...
SSE <-
  ...
  mutate(val = paste0(round(meanEst,3), " ", "(", round(sdEst,3), ")")) %>% 
