...
\section{SAO Diagnostics}

\hh{this needs a lit review of the methods used for diagnosing SAO models: you can start with describing the goodness-of-fit stats (`sienaGOF` function) and the meta analysis described in the RSiena manual, and then work on extending the literature cited and built on this.  }
