...
levels(seobama$label) <- c("", "Amy Klobuchar", "Bernie Sanders", "Harry Reid",
                           ...
                           "Lindsey Graham", "Marco Rubio", "Mitch McConnel", 
                           "Rand Paul", "Ted Cruz")
set.seed(56049382)
ggplot(data = seobama) + 
...
set.seed(34569234)
froms <- static_pall %>% select(id = from, microstep, x = from.x, 
        y = from.y, ms, .frame, size, color) %>% unique
tos <- static_pall %>% select(id = to, microstep, x = to.x, y = to.y, 
        ms, .frame) %>% unique
tos <- left_join(tos, froms)
nodedata <- rbind(froms, tos) %>% unique
nodedata <- nodedata %>% replace_na(replace = list(size = 1, 
        color = "#333333"))
to_ani <- ggplot() + 
