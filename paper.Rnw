...
<<senateplot, fig.cap='The collaboration network in the four senates during the Obama years, 2009-2016. Edges are shown only if the weighted propensity to cosponsor from one senator to another is greater than 0.25.', fig.height=3>>=
seobama <- read_csv("data/congress/senateobamapres_gsw_25.csv")
ggplot(data = seobama) + 
  geom_net(directed = T, labelon=F, arrowsize = .25, singletons= F, fiteach = T, linewidth = .25,
