...
The \texttt{RSiena} software that fits the models to data also comes equipped with the \verbatim{sienaGOF()} function for examining model fit. This function ``assess[es] the fit of the model with respect to auxiliary statistics of networks" \citep[p.~53]{RSienamanual}. Examples of the auxiliary statistics are the out- or indegree distribution on the nodes, with the option for users to create their own. 
