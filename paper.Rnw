...
#for(i in 0:55){
#plots[[i + 1]] <- 
  ggplot(data = adjmatall3 %>% filter(ms %in% c(1,3,5,7,9,11,13,43,45,47,49,51,53,55))) + 
  geom_tile(aes(x = to, y = from, fill = as.factor(val)), color = 'black') + 
  scale_fill_manual(values = c("white", "black")) + 
  scale_x_discrete(position = "bottom", labels = str_replace(order1, "V", "")) + 
  scale_y_discrete(labels = rev(str_replace(order1, "V", ""))) + 
