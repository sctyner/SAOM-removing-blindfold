...
Unlike other network models, SAOMs are not very well understood. They are relatively new, especially compared to the classic exponential random graph models, and they are not very tractable analytically with traditional methods. Likelihood functions turn into very complex objects due to the dependency structure inherent in the data. Therefore, computationally more tractable solutions are used to fit estimators, in particular, % Traditional modeling steps, like determining the likelihood function, are very complex. Because of this, 
...
%\hh{XXX what is an adjacency matrix? - explain at this point in mathematical terms} 
This type of visualization is also shown in Figure~\ref{fig:toyplots} for our toy example.  %\hh{XXX What is the exact logic here that Ghoniem used to come to the conclusion that edges in undirected graphs are over-represented? Why would directed graphs make that better? wouldn't that just add to the number of edges? } This, however, may actually be an advantage for \emph{directed} graphs, where exactly the correct number of edges is represented in a matrix visualization, but a node-link diagram may underrepresent the edge count with overlapping edges.  
...
For the network $x$ and each actor $i$ in the network, the number of times that an actor $i$ gets to change its ties, $x_{ij}$, to other nodes $j \neq i$ in the network is dictated by a \emph{rate function} $\rho(x, \mathbf{z}, \boldsymbol{\alpha})$. For this paper, we assume a simple rate function, $\rho(x, \mathbf{z}, \boldsymbol{\alpha}) = \alpha_m$ that is constant for all actors between observations at time $t_m$ to $t_{m+1}$. %\hh{XXX that means that the function is essentially just a rate parameter, right?}
...
When node $i$ is given the chance to change a tie, \hh{`we assume that they wish' sounds strange - don't assume wishes, state assumptions} we assume that they wish to maximize the value of their objective function $f_i(\boldsymbol{\beta}, x)$ plus a random element, $U_i(x)$, where the $U_i(x)$ are from \hh{XXX don't quote here - just state that Snijder suggest the use of a type 1 extreme value distribution on top of the objective function. XX What is the thought behind that? } ``the type 1 extreme value distribution (or Gumbel distribution) with mean 0 and scale parameter 1" \citep[p. 368]{snijders01}. This distribution, which is also known as the log-Weibull distribution, has probability distribution function, using $\mu$ for the mean parameter and $\sigma$ for the scale parameter, of

$$f(u|\mu, \sigma) = \frac{1}{\sigma}\exp\left\{-\left(\frac{u-\mu}{\sigma} + e^{-\frac{u-\mu}{\sigma}}\right)\right\}.$$


Using this distribution \hh{XXX I'm not following - which distribution do you mean by `this' distribution? The density above is the type 1 extreme value density. I got lost at the  ``objective function $f_i(\boldsymbol{\beta}, x)$ plus a random element, $U_i(x)$"} is convenient because it allows the probability the actor $i$ chooses to change its tie to actor $j$ in terms of the objective function alone. Let $p_{ij}(\boldsymbol{\beta}, x)$ be this probability. Next, write the network $x$ in its potential future state, where the tie $x_{ij}$ has changed to $1-x_{ij}$, as $x(i \leadsto j)$. Then, the probility that the tie $x_{ij}$ changes is 
...
\hh{This paragraph ends a bit abruptly - what is the future state probability used for?}
