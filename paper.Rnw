...
In Section~\ref{sec:samemoddiffdat}, we also fit models M1, M2, and M3 to the senate collaboration data for comparison. %Comparison of the parameter estimates of these models fit to the two different data sets is given in Table~\ref{tab:meanests}. 
...
clintonSenate %>% filter(source %in% target)
...
<<sigtesting, fig.show = 'hold', out.width='\\textwidth', fig.height=3, fig.cap="Significant effects for the two data sets, at the level of $\alpha = 0.10$ as calculated by the Wald-type test available in the \texttt{RSiena} software.">>=
...
<<heatmap, fig.align='center', fig.cap='A heatmap showing the empirical transition probabilities for the first microstep in 1,000 simulations. The acting ego node is on the y-axis, and the alter node is on the x-axis. There were many ties with empirical probability zero.', fig.height=4>>=
ansnullchains %>% 
  filter(period == 1) %>% 
  group_by(rep) %>% 
...
p1 <- ggplot(data = msall1_df2) + 
  ...
  annotate("text", x = c(-1, 84, 40), y = c(-30, -30, -60), label = c("Wave 1", "Wave 2", "Microsteps"), angle = c(90, 90, 0)) + 
