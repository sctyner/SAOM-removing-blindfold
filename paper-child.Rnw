...
%The model family, the model form, and the fitted model can each be visualized according to  %We begin by introducing the five models that we fit to our example data. 
...
After seeing the high correlation between parameter estimates in very different data contexts, we question the guideline that both parameters need to be included in the objective function. 
