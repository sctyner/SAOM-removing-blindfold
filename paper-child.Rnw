...
With these models, we can examine assumptionsm made about social networks, i.e. that people with common interests are more likely to become friends. For example, we may assume that high schoolers who drink heavily are more likely to be friends with others who drink heavily. SAOMs allow us to incorporate this behavioral information on shared interests into the modeling process.
...
In the CTMC, when actor $i$ can make a change, it chooses which tie $x_{i1}, \dots, x_{in}$ to change at random according to the probabilities $p_{ij}(\boldsymbol{\beta}, x, \mathbf{Z})$. We explore these probabilities further in Section~\ref{sec:algorithms}. The probabilities $p_{ij}$ combined with the rate parameters $\alpha_m$ fully characterize the underlying CTMC that models network change. For more details on CTMCs, see e.g. \citet{ctmc}. 
...
alldataviz <- ggplot(data = alldat,
  ...
  geom_net(fiteach = FALSE, directed=TRUE, layout.alg = 'fruchtermanreingold', size = 4, arrowsize = .25, arrowgap = .05, ecolour = 'grey40', labelon=T, labelcolour = 'white', fontsize = 2, vjust = .5, linewidth=0.5) +
...
M2: \quad f_i(x) = \beta_1s_{i1}(x) + \beta_2s_{i2}(x),
\end{equation}
...
\end{subfigure}
\caption{\label{fig:structures} The additional network effects included in our models fit to the friends data. On the left, a JTT between $i$ and $j$, who have different covariate values, and on the right, a N22 between $i$ and $j$.}
\end{figure}
...
\item creat the \textit{expected network} by including only the edges that appeared in more than 5\% of the simulations. 
\end{enumerate}
