...
numordersf$from <- as.numeric(as.character(numordersf$from))
numordersf$to <- as.numeric(as.character(numordersf$to))

p1 <- ggplot(data = numordersf, aes(y = -from, x = to)) + geom_tile(color = 'black', aes(fill = as.factor(value))) +
  scale_fill_manual(values = c('white', 'black')) +
  scale_x_continuous(breaks = 1:16) +
  scale_y_continuous(breaks = (1:16)*-1, labels =1:16)  +
  ThemeNoNet +
...
ggplot(data = probsDat,
       aes(y = factor(ego, levels = order1),
             x = factor(alter, levels = rev(order1)),
             fill = prob)) +
...
<<sigtesting, fig.show = 'hold', out.width='\\textwidth', fig.height=2, fig.cap="From Section~\ref{sec:allpossible}. Significant effects for the two data sets, at a significance level of 0.10 or lower as calculated by the Wald-type test available in the SIENA software.">>=
