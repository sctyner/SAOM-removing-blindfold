...
In addition to assessing model fit, we are also interested in model behavior so that we can choose better fitting models in the future. We examine model behavior by visualizing collections of models. There are many possible ways to collect SAOMs together, so 
