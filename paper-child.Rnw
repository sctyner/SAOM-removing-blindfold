...
Fully dynamic social network models, however, are more complicated: they consider both the network structure and its changes in time.  If we can capture this change mechanism over time, we can truly begin to model the underlying network dynamics that drive social connections. A comprehensive review of dynamic network models is presented in \citet{goldenberg09}. 
...
Since we chose the CTMC models for dynamic social networks, we would like to determine how well they fit data. We chose these models because they match our intuition about how social networks form in reality, and after we fit a model, we will want to assess its goodness-of-fit. In statistical network models generally, the lack of large-sample theory leads to a lack of goodness-of-fit tests \citep{goldenberg09}. Most methods for assessing goodness-of-fit for network models, therefore, involve comparing statistics computed on simulated networks to the same statistics computed on the data, for example in \citet{hunter2008}. More recently, \citet{gofschwein} used score-type tests for forward model selection in the CTMC model context. The \texttt{RSiena} software also contains methods for goodness-of-fit based on network statistics, such as outdegree distribution, where the data values are compared to simulated values as in \citet{hunter2008} and in \citet{RSiena}. % need to transition to model vis. % say something about how important visualization is in network analysis. HH: I think this transition is fine.

We propose new methods for goodness-of-fit and other diagnostics that use visualization as a supplement to computational methods. Network visualization, also called graph drawing, is a prominent subfield of network analysis with a rich history: early drawings of complete graphs date back to 1280 CE \citep{knuth2000}. We use this visualization foundation to combine modeling with visualization. Note that we do not propose new methods of visualizing networks, and for a complete review of the field of graph drawing we direct the reader to \citet{graphdrawing}. What is novel in our work is the use of existing visualization methods to diagnose network models with model visualization.

Model visualization or ``model-vis" complements traditional numerical model diagnostic tools to enrich our understanding and interpretation of statistical models \citep{modelvis}. We apply the three principles of model-vis: view the model in the data space, visualize collections of models, and explore the process of fitting the model, to the models and data introduced in Section~\ref{sec:exdatamods}.  By translating the model visualization work of \citet{modelvis} to the dynamic network model space, we provide network researchers with a new suite of tools for visualizing and diagnosing performance of their network models. Though we apply the tools to CTMCs because we find the models intuitive, the methods we use can be applied to any network model, static or dynamic. The visual diagnostics proposed here serve the same purpose for network models as residual plots for assessing goodness-of-fit in linear models.

The rest of this paper is structured as follows. In Section~\ref{sec:netintro}, we provide an overview of network data and visualization and give a more detailed description of CTMC models and how we fit them with \texttt{RSiena} \citep{RSiena}. In Section \ref{sec:exdatamods}, we introduce the motivating data and CTMC models we assess using model-vis. Then, in Section~\ref{sec:minds}, we explore the first step in model-vis by placing the models in the data space to determine how well the models fit the data. Section~\ref{sec:collectmodels} follows, in which we visualize collections of models to explore their behavior. Finally, in Section~\ref{sec:algorithms}, we visualize the underlying algorithms in the CTMC fitting process to gain insight into model fits. We conclude with a discussion in Section~\ref{sec:discussion}.


\section{Network Models and Visualizations} \label{sec:netintro}

%We provide a brief introduction to the structure of network data and common network visualization methods. \\

Network data across fields have similar structure: they consist of units of observation and connections between those units. In a social network, the units of observations, called \textit{nodes} or \textit{actors}, are people, while the connections, called \textit{edges} or \textit{ties}, are their relationships with each other. 
Other network data include inherent variables of interest on the nodes, such as age or gender, and the type of relationship (friends, coworkers, etc) that an edge represents. Dynamic networks are observed at two or more discrete points in time, where each observation of a network is often called a ``wave" of data.
...
The first net-vis method, the node-link diagram, represents nodes with points in two dimensions and then represents edges by connecting the points with lines. These lines can also have arrows on them indicating the direction of the edge for directed networks, as shown at left in Figure~\ref{fig:toyplots}. Because there is often no natural placement of the nodes, they are placed in 2D space at random, then adjusted with a layout algorithm.  Some commonly used algorithms, such as the Kamada-Kawai (KK) layout \citep{kamadakawai} and the Fruchterman-Reingold (FR) layout \citep{fruchterman-reingold}, are designed to mimic physical systems, drawing the graphs based on the ``forces" connecting them. For a full discussion of available node-link layout algorithms, we direct the reader to \citet{drawingalgs}.
In Figure~\ref{fig:toyplots}, we use the KK algorithm, and unless otherwise stated, all other node-link diagrams in this paper are laid out using the FR algorithm \citep{fruchterman-reingold}. 

\par The second method for net-vis uses the adjacency matrix of the network. The adjacency matrix of network $x$, $\mathbf{A}(x)$, is defined as:
...
\[ q_{bc} = \begin{cases}
   ...
   \end{cases}
\]
Thus, the rate of change between any two states, $x^b$ and $x^c$, that differ by only one tie $x_{ij}$, is the product of the rate at which actor $i$ gets to change a tie and the probability that the tie that will change is the tie to node $j$. The intensity matrix $\mathbf{Q}$ completely defines the CTMC model for network change. 

\subsubsection{Fitting Models to Data}\label{subsec:fitting}

To fit a CTMC to dynamic network data, we apply the \texttt{RSiena} software, which uses simulation methods to estimate parameter values using either MM or maximum likelihood (ML) estimation \citep{RSiena}. We chose to use MM estimation, so we describe that process here, while more information on ML estimation can be found in \citet{saomsml2010, SienaAlgs}. %because the theory behind it was established in \citet{saompaper}, while the maximum likelihood estimation methods were not fully established until \citet{saomsml2010}, though \texttt{RSiena} contains capabilities to use maximum likelihood estimation. We also use the score function method for estimating the derivatives of the expected values, as opposed to the finite differences method, both of which are outlined in detail in .
The underlying SIENA software uses a Robbins-Monro algorithm (see \citep{robbinsmonro}) to estimate the solution of the moment equation,
...
The full SIENA MM estimation algorithm operates in three phases which we briefly summarize here. Full description of the algorithmc can be found in \citet{RSienamanual} and \citet{SienaAlgs}. The first phase obtains initial estimates of the score functions for use in the second phase. The second phase is the Robbins-Monro algorithm, which results in parameter estimates via iterative parameter updates and subsequent simulation from the CTMC at each set of parameter values. The third phase uses the parameters estimated in phase two to estimate the score functions and covariance matrix of the parameters and carries out convergence checks. In Section~\ref{sec:algorithms}, we further explore these phases in the SIENA MM algorithm through visualization.
...
<<alldataviz, fig.cap="The small friendship network data we model throughout this paper. Each node represents a teenager, an edge is drawn when a girl identifies another as her friend. By comparing the node-link diagrams we see  that teenagers' drinking behavior changes -- drinking generally increases; darker nodes indicate more occurrences -- and the network becomes less densely connected over time. These are the mechanisms we hope to model with our chosen CTMCs.", out.width='.99\\textwidth', fig.height=3, results='hide'>>=
...
\noindent \textbf{Example Data:} To guide our visual exploration of CTMC models for dynamic networks, we use a canonical example in the social network literature: a subset of the 50 actor data set from the ``Teenage Friends and Lifestyle Study"  \citep{friendsdata}. We chose this data because it is a well-known and easy to understand example. We were all teenagers once, and we know the volatility and fragility of those relationships, as well as why they might change. We chose to only work with a subset of the data to make node-link diagrams easier to see and to make any changes in the network more noticeable. In the small data set, there are 16 teenage girls sampled over three years, who were asked about their friends and their behaviors, such as how often they smoke cigarettes or drink alcohol  \citep{friendsdata}.  Each girl was asked to identify her friends in the school to form a directed network. We visualize this data with a node-link diagram in Figure~\ref{fig:alldataviz}. Each girl is represented by a node and an arrow is drawn from her to each of the friends she has reported. Sometimes, two girls identify each other as friends, as shown by nodes 8 and 9 in the first sample, on the left in Figure~\ref{fig:alldataviz}. Often, the ties are not reciprocated, as shown by the edges from node 5 to nodes 2 and 3 in the first sample.
%\hh{HH: ground the figure - this is the first time you are showing this example. A node is ... an edge is ... , colour is ...}
We also color nodes in Figure~\ref{fig:alldataviz} according to each girl's self-reported drinking behavior: does not drink, drinks once or twice per year, drinks once per month, and drinks once per week. We can see that, as time passes, the students seem to drink more and become increasingly isolated into smaller groups. An analysis of this type of data with a CTMC model should capture these dynamics in a way that allows the researcher to draw conclusions about the nature of the network and the behavioral forces at play.
...
\end{equation}
where $s_{i1}(x)$ is the density network statistic and $s_{i2}(x)$ is the reciprocity network statistic for actor $i$ at the current network state $x$ (see Figures~\ref{fig:beta1} \& \ref{fig:beta2}). The second and third models, M2 and M3, contain one additional parameter each in the objective function. We chose the additional parameters because they were significant ($p$-value $<$ 0.05) according to a Wald-type test (\texttt{Wald.RSiena()}) provided in the \texttt{RSiena} software \citep{RSiena}. The model M2 contains an actor-level covariate parameter, and the model M3 contains an additional structural effect in the objective function.
...
ggplot(data = s1net, aes(from_id = from, to_id = to)) +
  geom_net(directed = T, labelon = T, arrow = arrow(type = "closed", length = unit(2,"mm")),
           labelcolour='white',vjust = 0.5, hjust =0.5, arrowgap = .15,
           size=6, fontsize = 5, ecolour="black",
           linetype = c("dotted", "solid", "solid","solid","solid", "solid","solid", "solid")) +
  expand_limits(x=c(-0.2,1.2), y=c(-0.1,1.1)) +
  ThemeNet +
  theme(legend.position = "none") + 
  labs(title = expression(paste(s[i1], "(x)")))
@
\caption{\label{fig:beta1}Demonstrating the outdegree effect $\beta_1$. $s_{i1}(x) = \sum_{j} x_{ij}$}
\end{subfigure}\hfill
\begin{subfigure}[t]{.45\textwidth}
...
@
\caption{\label{fig:beta2}Demonstrating the reciprocity effect $\beta_2$. $s_{i2}(x) = \sum_{j} x_{ij}x_{ji}$}
\end{subfigure}
\begin{subfigure}[t]{.45\textwidth}
...
@
\caption{\label{fig:jtt}Demonstrating a JTT, where the covariate is represented by the shape of the nodes.  $s_{i3}(x,z) = \sum_{j \neq h} x_{ij}x_{ih}x_{hj} \mathbb{I}(z_i = z_h \neq z_j)$}
\end{subfigure}\hfill
\begin{subfigure}[t]{.45\textwidth}
...
@
\caption{\label{fig:dad}Demonstrating a N22 between actors $i$ and $j$.  $s_{i4}(x) =  |\{j : x_{ij} = 0, \sum\limits_h x_{ih}x_{hj} \geq 2\}|$}
\end{subfigure}
\caption{\label{fig:structures} The network statistics whose corresponding effects are included in the models we fit to the friends data. In each drawing, the dotted tie is encouraged to for if the parameter is positive, and is discouraged otherwise.}
\end{figure}

\noindent where $s_{i3}(x,z)$ and $s_{i4}(x)$ are defined in Figures~\ref{fig:jtt} \& \ref{fig:dad}. These statistics are called the ``number of jumping transitive triplets" (JTT) and the ``number of doubly achieved distances two effect" %\hh{HH do you need the `two` in there? ST: Yes its in the documentation}
(N22), respectively in \citet{RSienamanual}. The JTT effect emphasizes triads formed between actors with \emph{different} covariate values, while the N22 effect emphasizes \emph{indirect} ties between actors.  
The four model parameters are visualized in Figure~\ref{fig:structures}, where the ties affected by the parameter value are represented by dotted lines. If the estimates of the parameters $\hat{\beta}_1, \dots, \hat{\beta}_4$ are positive, the model encourages the tie represented by the dotted line to form, while the opposite is true if the parameter estimate is negative. Mean estimates from 1,000 repeated fittings are given in Table~\ref{tab:meanests}, which we use when we simulate from the network models. 

%\hh{HH: include the names of the effects in the table -- otherwise it's really hard to make any sense of the numbers. ST: done. }
...
\begin{tabular}{llccc}
  \toprule
  Effect Name & Parameter & M1 & M2 & M3 \\
  \midrule
  Rate (time 1 $\rightarrow$ 2) & $\alpha_1$ & \Sexpr{SFE$M1[1]} & \Sexpr{SFE$M2[1]} & \Sexpr{SFE$M3[1]}  \\
   Rate (time 2 $\rightarrow$ 3) & $\alpha_2$ & \Sexpr{SFE$M1[2]} & \Sexpr{SFE$M2[2]} & \Sexpr{SFE$M3[2]}   \\
  Outdegree & $\beta_1$ & \Sexpr{SFE$M1[3]} & \Sexpr{SFE$M2[3]} & \Sexpr{SFE$M3[3]}  \\
  Reciprocity & $\beta_2$ & \Sexpr{SFE$M1[4]} & \Sexpr{SFE$M2[4]} & \Sexpr{SFE$M3[4]} \\
  JTT & $\beta_3$ & -- & \Sexpr{SFE$M2[5]} & --  \\
  N22 & $\beta_4$ & -- & --  & \Sexpr{SFE$M3[6]} \\
  \bottomrule
\end{tabular}

\caption{\label{tab:meanests} The means (standard deviations)\ of parameter values estimated from 1,000 repeated fittings of M1, M2, and M3 to the example data.}
\end{table}

The values in Table~\ref{tab:meanests} show us that all effects are very significant and relatively stable across the different models. Estimates of $\beta_1$ are negative in all three models, meaning that creating new outgoing ties is discouraged, while $\beta_2$ estimates are positive in all three models, meaning that creating reciprocated ties is encouraged. Thus in the context of teenage girls, we interpret this to mean they do not want to form new friends unless they are becoming friends with a girl who already thinks of them as a friend. The estimate of $\beta_3$ is positive, meaning that girls want to be close friends with their friend's friends even if they have different drinking behaviors. And finally, the estimate of $\beta_4$ is negative, so indirect relationships are discouraged. 
%\hh{HH: sentence of discussion - do we see the expected increase/decrease in the parameters?} \st{I do not understand.}
What we do not learn from the values in Table~\ref{tab:meanests} is anything about the goodness of fit of each of the models, or dependencies between the parameters and their estimates.

%We encourage interested readers to consult the \texttt{RSiena} manual for a complete definition and description of these and all other possible effects available to use in CTMC models \citep{RSienamanual}. 
% \hh{HH The next paragraph could be shortened some more to emphasize what we (want to) do. ST Done. I think!}
Our goal here is to assess the fit of models M1, M2, and M3, to the data, not to find the best model for this data, which is why we have kept both the data and the models very small. A complete analysis of the network would include all of the available data, including additional covariates related to smoking and other behaviors. The exact model structure and effects we choose are not as important to us as our goal of assessing their impact on model fit. We keep the data and model simple in order to apply the more complicated model visualization tools that we discuss in Sections~\ref{sec:minds}-\ref{sec:algorithms}. 
...
We define the \emph{data space} for our models as the combined data structures of the network: the node, edge, and time information. We use the \texttt{R} package \texttt{geomnet} to visualize the three data spaces together  \citep{geomnet}. The package draws the network using node-link diagrams, and node or edge data are then mapped to different visual features.  The color, size, and shape of the points can be used to represent variables in the node data, while the color, linewidth, and line type of the edges between nodes can be used to represent variables in the edge data. Finally, the networks observed at each time point are placed side-by-side, showing the network evolve over time. Bringing all network information together in \texttt{geomnet} allows us to show the entire data space at once. %To view temporal changes, we place the network at different timepoints side-by-side to see the evolution. By pulling all of this information together using \texttt{geomnet}, we can show the entire data space at once.

\subsection{Expectation Networks}

% this chunk doesn't produce any results. see next chunk.
...
p2 <- ggplot(data = filter(combinedavgs, cat == "1st Wave")) +
  geom_net(aes(from_id = from, to_id = to, linewidth= linewidth/3),
           directed = T, curvature = .2, labelon=T, size = 3,
           arrow = arrow(angle = 20, type = "open", length = unit(.05, "cm")), ecolour = "#969696",
...
<<averagenet, results='hide', out.width='\\textwidth', fig.cap='In the bottom left, the network at the first time point ($x(t_1)$), which is conditioned on in the model. In the bottom middle, the network at the second time point ($x(t_2)$). In the top row, the three expected networks for time point 2 ($\\bar{x}(t_2)$) according to model M1, M2, M3. The legend shows how edge width corresponds to frequency in the three expected networks.', fig.height=5>>=
...
combinedavgs %>% filter(!is.na(to)) %>% 
  ...
  geom_line(aes(x = from, y = to, size = linewidth), color = "black") +
  scale_size_continuous(name = "Edge frequency", range = c(0,1)) + 
  ThemeNet -> forlineleg  
legendlines <- cowplot::get_legend(forlineleg)
#set.seed(1356)
#set.seed(5)
set.seed(10) # this was my favorite seed
allp <- ggplot(data = combinedavgs) +
  geom_net(aes(from_id = from, to_id = to, linewidth= linewidth),
           directed = T, curvature = .2, labelon=T, size = 3, layout.alg = "fruchtermanreingold", 
           arrow = arrow(angle = 20, type = "open", length = unit(.1, "cm")), ecolour = "black", fiteach = F,
           color = "grey60", hjust = .5, vjust = .5, labelcolour = 'white', fontsize = 2, arrowgap = .04,
           singletons = T) +
...
% \hh{HH: we inconvenient question - could we use the same layout for all the networks (by using facetting instead of drawing individual plots). That would highlight the structure and changes to the structure better. I think, highliting the edges by making them darker and downweighing the nodes by making them lighter would also help. Something like (code in comment):
% % ggplot(data = combinedavgs) +
% %   geom_net(aes(from_id = from, to_id = to, linewidth= linewidth/3),
% %            directed = T, curvature = .2, labelon=T, size = 3,
% %            arrow = arrow(angle = 20, type = "open", length = unit(.2, "cm")), ecolour = "black",
% %            color = "grey60", hjust = .5, vjust = .5, labelcolour = 'white', fontsize = 2, arrowgap = .03,
% %            singletons = T) +
% %   labs(title = "Time 1") + ThemeNet +
% %   theme(plot.background = element_rect(color = 'black')) + facet_wrap(~cat)
% Hopefully this helps with the description below, because it's quite hard to read with all the numbers of nodes.
% }

\noindent The above method can be generalized and applied to any network model, static or dynamic, from which we can simulate, for any number of simulations from the network. In step 4, we chose 5\% because that is the typical significance level in statistical tests, and other values could be chosen. The expected networks from our three models of interest are shown in the top row of Figure~\ref{fig:averagenet}. Edges that appear with higher frequency are drawn with thicker lines. Edge frequency equal to one indicates that the edge was present in all 1,000 simulations used to create the expected network. %\hh{HH Give a definition of how edge frequency is defined in the plot - frequency of 1 means that this edge was present in all of the simulations, right?ST DONE. }
On the bottom row of Figure~\ref{fig:averagenet}, we show the data for comparison: $x(t_1)$ on the left, and $x(t_2)$ on the right. Visualizing the expected network in this way places models M1, M2, and M3, in the data space, and helps assess model fit.  For model M1 we can see that the overall structure of the average network is much more similar to $x(t_1)$  than to $x(t_2)$ . In  $x(t_1)$ and each of the three $\bar{x}(t_2)$, there are two clusters of nodes: $\{2,3,4,5,6,12,13,15\}$ and $\{7,8,9,10,11,14\}$. The expected networks, however, do not resemble  $x(t_2)$, which they are supposed to represent. In $x(t_2)$, there is one small, strongly connected cluster and one larger, sparsely connected cluster, with four unconnected nodes. Because the expected networks $\bar{x}(t_2)$ more closely resemble $x(t_1)$ than $x(t_2)$, we take this as evidence that none of the models are a good fit to the data.
Model M2, which included the transitive triplet parameter, results in more strongly connected components, as shown by the increased number of edges and the thicker lines between nodes. The lack of fit in all models is clear:  not one of the three model expectations show node 16 gaining ties as it does in $x(t_2)$, nor do they show nodes 4, 7, and 15, becoming isolated. In model M2, however, the ties to node 4, 7, and 15, appearless frequently than in model M1 or model M3, and the nodes $\{10,11,14\}$ are also more often connected, as they are in wave 2 of data. This suggests that, despite the fact that neither of the three models is a good fit, M2 may be the best fitting model of the three. In a complete data analysis, we can use this information to select more models for analysis that utilize transitivity and covariate effects like $\beta_3$, as opposed to structural and indirect effects like $\beta_4$.  
...
<<summary2, fig.height=2.5, fig.cap="Residual networks between the expected network from model M1 $\\bar{x}(t_2)$ and $x(t_1)$  and  $x(t_2)$. Some edges appear in both the data and the expected network $\\bar{x}(t_2)$ (dotted lines), while others only appear in the data (solid lines) or only in the expectation (dashed lines). Width of dotted and dashed edges represent the frequency of appearances in the 1,000 simulations.">>=
...
p1 <- ggplot(data = t1_1) +
  geom_net(aes(from_id = from, to_id = to, linewidth= linewidth, linetype = color2),
           directed = T, curvature = .2, labelon=T, size = 3, ecolour = "black",
           arrow = arrow(angle = 20, type = "closed", length = unit(.15, "cm")), 
           color = "gray60", hjust = .5, vjust = .5, labelcolour = 'white', fontsize = 2, arrowgap = .03, layout.alg = "fruchtermanreingold",
           singletons = T) +
  scale_linetype_manual(values =c("longdash", "dotted") ) + 
  labs(title = "Residual between Time 1 Data &\n M1 Expectation") + ThemeNet +
  theme(plot.background = element_rect(color = 'black'), legend.position = "none")

pforleg <- ggplot(data = mtcars) + 
  geom_line(aes(x = mpg, drat, group = as.factor(cyl),
                linetype = as.factor(cyl))) + 
  geom_line(aes(x = mpg, drat, size = vs)) + 
  scale_linetype_manual(values = c("dotted", "solid", "longdash"), name = "Tie is...", labels = c("in data, in\nexpectation", "in data,not in\nexpectation", "not in data, in\nexpectation")) +
   scale_size_continuous(name = "Edge Frequency", breaks = 1:4/4, range = c(.25, 1)) + 
  theme_grey() + 
...
p2 <- ggplot(data = t1_2) +
  ...
  labs(title = "Residual between Time 2 Data &\n M1 Expectation") + ThemeNet +
  theme(plot.background = element_rect(color = 'black'), legend.position = "none")
legendsumm <- cowplot::get_legend(pforleg)
ggdraw() + draw_plot(p1, x = 0, y = 0, height = 1, width = .38) +  
  draw_plot(p2, width = .38, x = .39, y = 0, height = 1) + 
  draw_grob(legendsumm,x=.78, y=.4, width = .2 , height = .2,  scale = .1, clip = "on") 
@



Another way to view our models in the data space is with \textit{residual networks} between the expected network and the data, as shown in Figure~\ref{fig:summary2}. We created these networks by computing the residual between $\bar{x}(t_2)$ and $x(t_1)$, $\mathbf{A}(x(t_1))-\mathbf{A}(\bar{x}(t_2))$, and the residual between $\bar{x}(t_2)$ and $\bar{x}(t_2)$, $\mathbf{A}(x(t_2)) - \mathbf{A}(\bar{x}(t_2))$ for model M1. The lack of model fit is apparent on the right in Figure~\ref{fig:summary2}, where the dashed edges represent ties that only show up in the expected network, and the solid edges represent ties in the data that the model fails to capture.  Dotted edges are in both the expectation and the data and represent where the model is correct.  On the left in Figure~\ref{fig:summary2}, we see the residual network between $x(t_1)$ and the expected network. We see mostly dotted edges, meaning that the simulated networks consist primarily of edges in $x(t_1)$, plus some new edges represented by the dashed lines that are not in the data. The unobserved ties are all reciprocating ties in the data, so the model is predicting more reciprocity between actors than there actually is. On the right in Figure~\ref{fig:summary2}, we see the residual network between $x(t_2)$ and the expected network. Because the model is simulating  $x(t_2)$, we would hope to see very few differences between the expected network and the data. However, there are six solid edges, which are in the data but do not appear at all in the expected network, and there are many dashed edges that appear in the expected network but are not in $x(t_2)$. Although the residual network shows some edges that appear in both the expected network and $x(t_2)$, the majority of edges in the expected network are not in the observed data, and there are several edges in the data missing from the expectation. We therefore conclude that the model M1 is a poor fit to our example data, which we have seen very clearly by viewing the model in the data space. As with the expected network, we can apply the residual network visualization other network models to determine how closely the model fits the data. 
...
<<img, out.width = "\\linewidth", fig.height=7.5>>=
...
knitr::include_graphics("img/plot-matrix.png")

@
\caption{\label{fig:corplots} Diagnostic pairs plot. Relationships between parameters common to all models. Densities and scatter plots are colored by model. \nocite{ggally}}
\end{subfigure}
\caption{\label{fig:distests} Distribution of fitted parameter values for our three models. The inclusion of $\beta_3$ clearly has an effect on the distributions of the rate parameters, $\alpha_1$ and $\alpha_2$, and on the other objective function parameters $\beta_1, \beta_2$, while the effect of $\beta_4$ is not as noticeable.}
\end{figure}

\noindent \textbf{Fitting the model repeatedly:} The simulations required to fit the CTMC network models lead to inherent variability in the fitted values. To examine this variability, we fit models M1, M2, and M3 to the friendship network 1,000 times each. The resulting means and standard deviations of the estimates are given in Table~\ref{tab:meanests}, and the distributions of all parameter estimates are shown in Figure~\ref{fig:distests}.  Figure~\ref{fig:corplots} shows a \textbf{diagnostic pairs plot}, in which we can see that the addition of $\beta_3$ to the model results in very different estimates the of the other four parameters, $\alpha_1$, $\alpha_2$, $\beta_1$, and $\beta_2$. In fact, the diagnostic pairs plot reveals that the distributions of $\alpha_1$ and $\beta_1$ for M2 do not overlap at all with the distribution of the same parameters from model M1. When $\beta_3$ is included, its estimate is positive, meaning that friendships between two girls with different drinking behaviors tend to form when there is third girl who is already friends with the other two. The inclusion of $\beta_3$ also leads to increases in the estimates of $\alpha_1, \alpha_2$. Thus, when modeling the transitive triplet behavior in this way, the model concludes that girls change friends more frequently. It is not clear, however, that the addition of a parameter to the objective function should affect the estimates of the rate parameters. 

On the other hand, the inclusion of the indirect tie parameter, $\beta_4$, does not lead to such drastically different estimates of other model parameters.  The mean estimate of $\beta_4$ is large and negative, which suggests that indirect ties are strongly discouraged from forming. This is fairly intuitive given the context: teenage girls likely want to have direct friendships in tightly knit groups, so indirect relationships are not common.
%This strong and possibly problematic relationship between rate parameter estimates and objective function parameters is further explored in the Supporting Information. When visualizing the results of the same models fit many times to the same data, we can to see potential model behavior problems right away.   
Finally, in Figure~\ref{fig:corplots}, we demonstrate the relationship between each of pair of parameters common to all three models. The strongest correlation within each model is between $\beta_1$ and $\beta_2$, with absolute correlation greater than 0.90 in all three models. The $\beta_1$ parameter is also highly correlated with the $\beta_3$ parameter within model M2 ($\rho = $ \Sexpr{round(corm2[5, 3],3)}), but it is not as highly correlated with the $\beta_4$ parameter in model M3 ($\rho = $ \Sexpr{round(corM3[5, 3], 3)}). The inherent relationship between effects is a known problem in network analysis \citep{goldenberg09}, and the diagnostic pairs plot can be used for any network model where parameters are estimated with MCMC methods. 
...
The third and final model visualization principle, ``explore the process of model fitting," is crucial to understanding the CTMC models \citep{modelvis}. When fitting models in \texttt{RSiena}, several simulation processes are hidden from the user. We refer to one of these hidden pieces in the SIENA MM algorithm as the \textit{microstep process}, also called the ministep process in \citet{SienaAlgs}. The microstep process is a series of state changes in the CTMC as described in Section~\ref{sec:saoms}. At each microstep, an ego node is selected to change, and then randomly makes one change in its ties according to to the probabilities $p_{ij}$ defined in Equation~\ref{eq:pij}. The microstep process is a key piece of the model fitting procedure. During modeling, the process proceeds until the network differs from the $x(t_m)$ by exactly $C_m$, defined in Equation~\ref{eq:cdiff}. Between two network observations $x(t_m)$ and $x(t_{m+1})$, there can be dozens, hundreds, or even thousands of microsteps, depending on the size of the network and the value of $C_m$. We want to simulate and view these steps in order to better understand the behavior of the CTMC model. By default, the software discards the microsteps, which is practical and efficient if the goal is to fit a model to data, obtain parameter estimates, and draw conclusions or make predictions. But in order to understand how the models are fit, we need to extract and explore the different steps of the process, which is possible in \texttt{RSiena} with some small changes to the default settings. We visualize the microsteps to visually explore the algorithms, so that we are visualizing the model fitting process, and not only the model fitting result.  
...
<<makinggif, fig.cap='Some frams from the animation at \\url{https://vimeo.com/240089108}. The selected edges and nodes are emphasized by changing size and color, then they fade in or out while the nodes change color and shrink to blend in with the other nodes. Frames 15-28 show edge destruction, while frames 71-84 show edge creation. A spring layout is used \\citep{sna}.', message = FALSE, warning = FALSE, fig.height=2.5>>=
...
to_ani <- ggplot() +
  geom_curve(data = static_pall, aes(x = from.x,
        ...
        length = unit(0.15, "cm"), type = "closed" )) +
...
To visualize the underlying process, we animate the microsteps that form the CTMC from $x(t_1)$ to $x(t_2)$ for the example data.  The microstep animation, some frames of which are shown in Figure~\ref{fig:makinggif}, represents one possible path from $x(t_1)$ to $x(t_2)$ according to the model M1. We show each change in the network as follows: we first increase the size and change the color of the ego node to focus attention to its ties. Then, the changing tie either fades in or fades out. If there are no changes at a particular microstep, the microstep does not appear. Some frames of the animations are shown in Figure~\ref{fig:makinggif}, and the full movie, which we created using \texttt{tweenr} and \texttt{gganimate} can be viewed at \url{https://vimeo.com/240089108} \citep{tweenr, gganimate}. Movies similar to this animation were used to visualize the changes of dynamic networks in \citet{dynnetviz}. Frames 15-28 of Figure~\ref{fig:makinggif} show a tie that is destroyed, while frames 71-84 show a tie that is created. For both changes, the ego node is emphasized with color and size. When a tie is removed, it is first emphasized with the same color and size as the node. As the animation proceeds, the tie shrinks and disappears as the ego node shrinks and changes color to match the others. If a tie is being added, it appears red from nothing, grows large for emphasis, then changes color and size to match the rest of the edges. In this network animation, we show the modeled steps of the unobserved CTMC process between two observed networks. We see each part of the hierarchical model: first, the selection of the ego node via the rate function, then the result of the actor maximizing its objective function by either deleting or adding a node. The layout of the network also adjusts as edges are removed or added, demonstrating how the overall network structure changes with each of the individual tie changes. Combining the microstreps in this animation makes the CTMC process the focus, demonstrating the underlying change mechanism of the dynamic network according to the model. 

With the node-link animation, we can see how the model behaves. Some tie changes appear to alter the overall network structure, as shown by the change in the layout of the nodes, while others seem to be inevitable, such as an added reciprocal tie between two nodes in a large connected group that does not change the layout. Other edges are created in one step and destroyed the next.  When watching the animation, we are watching the model in action, which helps us better understand its behavior. This same animation method can be applied to any network model which changes one tie at a time. For example, the Erd\"{o}s-R\'{e}nyi random graph model is a series of tie additions, and we could animate the creation of a network from this model. Unlike CTMCs, however, Erd\"{o}s-R\'{e}nyi random graph models only create ties \citep{goldenberg09}. Regardless of the network model under consideration, animating the node-link diagram to view how the network changes gives insight into the model behavior and the process of model fitting. % need more about what we learned. 
...
We continue to use animation to view the changing structure of the network via adjacency matrix visualizations. Two adjacency matrix visualizations are shown in Figure~\ref{fig:adjmatorder}. At left in Figure~\ref{fig:adjmatorder}, we use the node ID to order the rows and columns. This arbitrary ordering does not easily convey the underlying structure of the network, so we use \textit{matrix seriation} to reorder the rows and columns of the adjacency matrix visualization to make the structure of the network more obvious \citep{seriation}. %This lack of perceived structure would contribute to further confusion as the network chages, so we adjust the ordering so that the viewer can better perceive the structure of the network. This process is known as matrix seriation 
...
adjacency matrix visualization clearly shows the two distinct connected components in the first network, which are difficult to find in the visualization ordered by node ID at left. We use the seriated matrix in an adjacency matrix animation of the microstep process, which is available at \url{https://vimeo.com/240092677}. Some frames of the video are shown in Figure~\ref{fig:adjmatani}. In the animation, a square appears or disappears when the edge it represents appears or disappears in the microstep process. %Through this animation, we can see edges appearing, and then later on disappearing. 
This animation, unlike the previous one, shows the microsteps where the chosen node does not make a change. Starting at frame 0 (the first network observation $x(t_1)$), there are two clearly connected components: one in the bottom left, and one in the top right. With each frame, we see the network evolve: sometimes ties are removed right after they are created, and sometimes several ties are removed or created in a row. By the end, the bottom left component has become more sparsely connected, while the top right component has shrunk, but remains closely connected. In contrast to the node-link animation, the adjacency matrix animation shows how the algorithm explores the space of all possible networks, because all possible edges are represented in each frame of the adjacency matrix animation.  It is also easier to see the connected components of the graph in this animation because we have clearly grouped them with matrix seriation. As with the node-link animation, the adjacency matrix animation could also be used for other dynamic network models. Because the adjacency matrix emphasizes edges, this animation method is best for large directed networks whose structure is easier to see in an adjacency matrix visualization than in a node-link visualization \citep{adjmatviz}. Both animation methods demonstrate model behavior that we would not have seen with traditional visualizations or computations.
% need more oomph 

<<adjmatani, fig.cap='A selection of frames from the adjacency matrix visualization animation for one series of microsteps. The node labels are removed to declutter the figure. Complete video available at \\url{https://vimeo.com/240092677}.', warning = FALSE, message = FALSE, results='hide', fig.height=3, out.width="\\linewidth">>=
...
% \end{figure}
% 
% Finally, we illuminate the microstep process by visualizing the theoretical and observed transition probabilites for the first microstep in the process. These probabilities are defined in Equation~\ref{eq:pij}. We examine the first step because each microstep is dependent on its predecessor, and thus only the first steps can be compared across simulations.  
...
%\hh{It would make the discussion easier, if all of the main plots we introduce had a name - like expectation networks, difference (or residual?) networks, ... maybe 'parameter diagnostics' for the second approach? and change networks for the last one? }

%\hh{We started with the three pronged approach of visualization ... let's bring it back to that.}
%\hh{Another point is a call for action: along the lines of now we need to use it!
%Visualization of network models should become second nature, similar to linear model diagnostics, we should have a suite of visualizations next to diagnostic values ready to launch. Ease of use?}

We have introduced four diagnostic visualizations, the expected network, the residual network, the node-link animation, and the adjacency matrix animation, to complement existing numerical diagnostics for network models. These visual diagnostics give insights into the network model fit that purely numerical methods do not. 
%We have used several visualization methods, some old and some new, to diagnose and understand network models in new ways. 
Though we have applied these methods to CTMC models for dynamic social networks, all of the diagnostics presented can be applied to other dynamic network models and most can also be applied to any static network model. 
By following the guiding principles of model visualization, viewing the model in the data space, visualizing collections of models, and looking at the underlying algorithms, we gained insight into and appreciation for a complicated and flexible family of network models \citep{modelvis}. The expected network and the residual network let us view the model in the data space, and we are then able to easily assess model goodness-of-fit. Visualizing the residual network between the data and the expected network also helps assess model fit by clearly showing us exactly where the model fits the network well and where it lacks fit.  Exploring collections of models give us the opportunity to learn more about the large number of parameters possible in CTMC models.  
The diagnostic pairs plot visualizes collections of models, giving us insight into the marginal distribution of parameters and their covariance structure. %and we also visualized relationships between parameters in the model that require further investigation. 
Finally, we used animation of node-link diagrams and the adjacency matrix visualizations to explore the underlying algorithms and expand our understanding of the model's stochastic behavior. All in all, this three-pronged model-vis attack has uncovered many properties of CTMC models for social networks and shows great potential for application to other network models. 

Using existing \texttt{R} packages, we have demonstrated how to apply model-vis to one family of models for social network analysis. The models and data we have used are multilayered and multidimensional, making them more complex than common statistical models, such as linear models. As models become more complicated, visualization becomes more important to model diagnosis because increasing complexity makes it harder to adequately summarize model fit only using numerical methods. Even more complex models, such as those for machine learning, can also benefit greatly from model visualization techniques without a lot of additional effort. The visualization methods we have used, such as the node-link diagram, are not new but we have used them in conjuction with model simulation to learn a great deal about our models of interest.  

We  explored a few simple models from the complicated and multilayered CTMC model family for social networks, %The \texttt{RSiena} software is incredibly powerful, and we can fit much more flexible hierarchical models than we have examined here. 
but many more than three parameters can be included in the objective function, and more than one actor-level covariate can be included at either level in the hierarchical model. We can also model the rate of change differently for each actor if, for example, we think an actor's covariate affects its rate of change. % <- reverse this section 
With increased model complexity, there is more potential for adverse effects, such as the correlation between parameters that we saw in Section~\ref{sec:collectmodels}. Armed with model-vis techniques, researchers can gain a better understanding of these and other adverse effects and can then find ways to alleviate them the model. In addition, extensions of the CTMC model can also model behavior change of the actors in the network, which we haveused as model covariates. Because of its flexibility, the CTMC family of models for dynamic networks has immense potential for additional model visualization applications.

While the data we have used are quite small by network data standards, which favors node-link views, we also discuss adjacency matrix visualization, which scale up to large networks.  %Thus, some of the visualizations we have presented, such as the small node-link diagram animation, may not scale up to larger networks. 
Additionally, interactive and/or three-dimensional graphics, which we did not explore here, could be used for larger networks. Interactivity is the state-of-the-art in data visualization, and network visualization is just starting to catch on \citep[see e.g.\ ][]{refinery}. \citet{modelvis} also relied on interactive tools like GGobi \citep{STLBC03} for interactivity, and for networks, there is some interactive support with the package \texttt{plotly}  built into \texttt{geomnet} \citep{plotly, geomnet}. 

The principles of model-vis provide a solid foundation on how to use visualization to gain insight into more complex models, such as network models. Our model-vis application has uncovered many properties of CTMC models for dynamic social networks, and potential abounds for application of model-vis in other areas of network analysis.   

{\centering \subsection*{Online supplemental material:}}

Much of the code used to create the visualizations in this paper has been packaged and made available online at \url{https://github.com/sctyner/netvizinf} and \url{https://github.com/sctyner/geomnet} (also on CRAN). Additional, non-packaged code can be found at \url{http://bit.ly/ctmc-model-viz}. 
