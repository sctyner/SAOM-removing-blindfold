...
\st{Because longitudinal network data consist of three different ``spaces" of data, viewing the model in the data space can depend on which aspect of the \textit{model} and the \textit{data} we are interested in viewing. %The SIENA software includes a method for modelling covariate change in the network
