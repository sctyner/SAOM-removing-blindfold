...
\noindent \textbf{Varying model settings:} We now view collections of models by varying model settings. We have done this already by choosing three different models M1, M2, M3 to fit to our example data sets. To explore the behavior of these three models, we simulate from them. Using the parameter values given in Table~\ref{tab:meanests} for the friendship data, we simulated 1,000 observations from each of the three models and created expected network visualizations from models M1, M2, and M3 using the same method as in Section~\ref{sec:minds}. These averages are shown in Figure~\ref{fig:averagenet} in the Appendix. We see that all models are failing to capture the structure of the second wave. %In Section~\ref{sec:samemodsamedat}, we fit these three models to the data 1,000 times, and examine the parameter densities. Here, . For simulation, we used the mean parameter values from the 1,000 fits of the corresponding model. 
