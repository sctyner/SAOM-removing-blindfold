...
ggplot(data = jTT, aes(from_id = from, to_id = to)) +
  ...
  #      )
@
\caption{\label{fig:jtt}Realization of a JTT, where $i$ is the ego, $j$ is the alter, and $h$ is the intermediary. The covariate is represented by the shape of the nodes. $s_{i3}(x,z) = \sum_{j \neq h} x_{ij}x_{ih}x_{hj} \mathbb{I}(z_i = z_h \neq z_j)$}
\end{subfigure}\hfill
\begin{subfigure}[t]{.45\textwidth}
